...
class SelectionSort(Scene):
    def construct(self):
        ...
        for i in range(n - 1):
            ...
            self.play(
                min_circle.animate.move_to(
                    squares[i].get_bottom() + DOWN * 0.3 + LEFT * 0.15
                ),
                run_time=0.3,
            )
            ...
            for j in range(i + 1, n):
                ...
                self.play(j_label.animate.become(new_j_label), run_time=0.3)

                # Move current_circle to current position
                self.play(
                    current_circle.animate.move_to(
                        squares[j].get_bottom() + DOWN * 0.3 + RIGHT * 0.15
                    ),
                    run_time=0.3,
                )
                ...
                if int(squares[j][1].text) < int(squares[min_idx][1].text):
                    ...
                    self.play(
                        min_circle.animate.move_to(
                            squares[min_idx].get_bottom() + DOWN * 0.3 + LEFT * 0.15
                        ),
                        run_time=0.3,
                    )
                else:
        ...
        current_circle.animate.move_to(
            squares[0].get_bottom() + DOWN * 0.3 + RIGHT * 0.15
        )
        for square in squares:
            self.play(
            ...
            self.play(square.animate.scale(1 / 1.1), run_time=0.1)
